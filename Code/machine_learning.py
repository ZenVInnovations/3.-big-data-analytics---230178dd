...
try:
    ...
    # Drop the 'dt' column as it contains non-numeric date values
    df = df.drop('dt', axis=1)

    # Convert categorical data to one-hot encoding
    df = pd.get_dummies(df, columns=['City', 'Country'])  # Replace 'City' and 'Country' with your categorical columns

    # Split the dataset into features (X) and target (y)
    X = df.drop('standardized_numeric', axis=1)
